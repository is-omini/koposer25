...
class DreamMountainTitleBar : public QWidget {
public:
    explicit DreamMountainTitleBar(ExplorerFiles *projectExplorer, DevelopmentTab *tablDeveloppement, QWidget *parent = nullptr) : QWidget(parent) {
        setStyleSheet(
            "background-color: #2c2d30;"
        );
        setFixedHeight(40);

        QHBoxLayout *layoutExplorerProjectActionButtons = new HorizontalBoxLayout(this);
        layoutExplorerProjectActionButtons->setContentsMargins(4,4,4,4);
        ///
        Button *windowClose = new Button();
        windowClose->setStyleSheet(R"(
        	QPushButton {
        		width: 12px;
        		height: 12px;
        		padding: 0px;
        ...
        QObject::connect(windowClose, &Button::clicked, []() {});
        layoutExplorerProjectActionButtons->addWidget(windowClose);
        ///
        ///
        Button *windowMini = new Button();
        windowMini->setStyleSheet(R"(
        	QPushButton {
        		width: 12px;
        		height: 12px;
        		padding: 0px;
        ...
        QObject::connect(windowMini, &Button::clicked, []() {});
        layoutExplorerProjectActionButtons->addWidget(windowMini);
        ///
        ///
        Button *windowFullScreen = new Button();
        windowFullScreen->setStyleSheet(R"(
        	QPushButton {
        		width: 12px;
        		height: 12px;
        		padding: 0px;
        ...
        QObject::connect(windowFullScreen, &Button::clicked, []() {});
        layoutExplorerProjectActionButtons->addWidget(windowFullScreen);
        ///
        ...
        QObject::connect(projectImportFolder, &Button::clicked, [projectExplorer, this]() {
        ...
        QObject::connect(editorCodeToggleViewButton, &Button::clicked, [tablDeveloppement]() {
            tablDeveloppement->toggleViewCodeEditor();
        });
        ...
        QObject::connect(webViewToggleViewButton, &Button::clicked, [tablDeveloppement]() {
            tablDeveloppement->toggleViewWebView();
        });
...
DreamMountain::DreamMountain(QWidget *parent) : QMainWindow(parent) {
	blueColor = "#3477f7";
	...
	setWindowTitle("DreamMountain : InDevelopment");
    ...
    setCentralWidget(cw);
    cw->setStyleSheet("#root { background-color: "+inputBackgoundColor+"; }");

	QVBoxLayout *mainWindow = new VerticalBoxLayout(cw);
    ...
    bloc->setStyleSheet("background-color: "+windowBackgoundColor+";");
    //


	tablDeveloppement = new DevelopmentTab(this, codeEditorInput, vueWeb, filesActiveList, Qt::Horizontal);
	projectExplorer = new ExplorerFiles(this, codeEditorInput, vueWeb, tablDeveloppement);

	QWidget *actionBar = new DreamMountainTitleBar(projectExplorer, tablDeveloppement, this);
...
void DreamMountain::setCurrentPath(QString path) {
	qDebug() << path;
	currentPath = path;
	setWindowTitle("DreamMountain : InDevelopment : "+currentPath);
}